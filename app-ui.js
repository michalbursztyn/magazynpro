...
function getManagedModalEntry(backdropId) {
  return _managedModalStack.find(entry => entry.backdropId === backdropId) || null;
}

function syncManagedModalBackground() {
  document.querySelectorAll('[data-managed-modal-inert="1"]').forEach(element => {
    element.removeAttribute('inert');
    element.removeAttribute('data-managed-modal-inert');
  });

  const topModal = _managedModalStack[_managedModalStack.length - 1] || null;
  if (!topModal) return;

  Array.from(document.body?.children || []).forEach(element => {
    if (element === topModal.backdrop || element.contains?.(topModal.backdrop) || element.matches?.('script, .toast-host')) return;
    if (!element.hasAttribute('inert')) {
      element.setAttribute('inert', '');
      element.setAttribute('data-managed-modal-inert', '1');
    }
  });

  let activeBranch = topModal.backdrop;
  while (activeBranch?.parentElement && activeBranch.parentElement !== document.body) {
    Array.from(activeBranch.parentElement.children || []).forEach(sibling => {
      if (sibling === activeBranch || sibling.matches?.('script, .toast-host')) return;
      if (!sibling.hasAttribute('inert')) {
        sibling.setAttribute('inert', '');
        sibling.setAttribute('data-managed-modal-inert', '1');
      }
    });
...
function getManagedModalFocusableElements(entry) {
  const root = entry?.dialogRoot;
  if (!root) return [];
  return Array.from(root.querySelectorAll(MANAGED_MODAL_FOCUSABLE_SELECTOR)).filter(element => (
    !element.closest('.hidden')
    && element.getAttribute('aria-hidden') !== 'true'
  ));
}

function focusManagedModal(entry) {
  if (!entry) return;
  const preferred = entry.initialFocusId ? document.getElementById(entry.initialFocusId) : null;
  const focusable = getManagedModalFocusableElements(entry);
  const target = preferred && !preferred.disabled && !preferred.closest('.hidden')
    ? preferred
    : (focusable[0] || entry.dialogRoot);
  if (!target) return;
  if (target === entry.dialogRoot && !target.hasAttribute('tabindex')) target.setAttribute('tabindex', '-1');
  target.focus?.({ preventScroll: true });
}

function openManagedModal(options = {}) {
  ...
  const existingIndex = _managedModalStack.findIndex(entry => entry.backdropId === backdropId);
  const existingEntry = existingIndex >= 0 ? _managedModalStack.splice(existingIndex, 1)[0] : null;
  const dialogRoot = panel.closest('[role="dialog"]') || panel;
  const entry = {
    backdropId,
    panelId,
    backdrop,
    ...
    initialFocusId: String(options?.initialFocusId || ''),
    requestClose: typeof options?.requestClose === 'function' ? options.requestClose : null,
    restoreFocus: existingEntry?.restoreFocus || document.activeElement
  };
  ...
  syncManagedModalBackground();
  requestAnimationFrame(() => focusManagedModal(entry));
  return true;
}

function closeManagedModal(backdropId) {
  const index = _managedModalStack.findIndex(entry => entry.backdropId === backdropId);
  const entry = index >= 0 ? _managedModalStack[index] : null;
  const backdrop = entry?.backdrop || document.getElementById(backdropId);
  const panel = entry?.panel || null;
  const wasTopmost = index === _managedModalStack.length - 1;

  if (index >= 0) _managedModalStack.splice(index, 1);
  backdrop?.classList.add('hidden');
  backdrop?.setAttribute('aria-hidden', 'true');
  panel?.classList.add('hidden');
  if (entry?.bodyClass && !_managedModalStack.some(item => item.bodyClass === entry.bodyClass)) {
    document.body.classList.remove(entry.bodyClass);
  }
  syncManagedModalBackground();

  if (wasTopmost) {
    requestAnimationFrame(() => {
      const nextTop = _managedModalStack[_managedModalStack.length - 1] || null;
      if (nextTop) {
        focusManagedModal(nextTop);
        return;
      }
      const restoreTarget = entry?.restoreFocus;
      if (restoreTarget?.isConnected && !restoreTarget.closest?.('.hidden')) {
...
function requestCloseTopManagedModal() {
  const entry = _managedModalStack[_managedModalStack.length - 1] || null;
  if (!entry) return false;
  if (entry.requestClose) entry.requestClose();
  else closeManagedModal(entry.backdropId);
  return true;
}

window.openManagedModal = openManagedModal;
window.closeManagedModal = closeManagedModal;
window.requestCloseTopManagedModal = requestCloseTopManagedModal;
...
function renderTablePagination(tableSelector, tableKey, meta) {
  ...
  mount.innerHTML = `
    <div class="table-pagination-inner" aria-label="Paginacja tabeli">
      <button type="button" class="table-pagination-btn" aria-label="Poprzednia strona" ${page <= 1 ? 'disabled' : ''} data-action="changeTablePage" data-table-key="${escapeHtml(tableKey)}" data-delta="-1">&lt;</button>
      <span class="table-pagination-label">Strona ${page} z ${totalPages}</span>
      <button type="button" class="table-pagination-btn" aria-label="Następna strona" ${page >= totalPages ? 'disabled' : ''} data-action="changeTablePage" data-table-key="${escapeHtml(tableKey)}" data-delta="1">&gt;</button>
    </div>
  `;
}
...
let _adjustmentHistoryCache = {
  historyRef: null,
  createdLotIds: new Set(),
  positiveChangesByDateAndSku: new Map()
};

function getAdjustmentHistoryCache() {
  const history = Array.isArray(state?.history) ? state.history : [];
  if (_adjustmentHistoryCache.historyRef === history) return _adjustmentHistoryCache;

  const createdLotIds = new Set();
  const positiveChangesByDateAndSku = new Map();
  ...
  history.forEach(ev => {
    ...
    itemGroups.forEach(items => {
      items.forEach(item => {
        ...
        if (createdLot && typeof createdLot === 'object') {
          [createdLot.lotId, createdLot.id, createdLot.lot_id].forEach(rawId => {
            const lotId = normalizeLotIdForCompare(rawId);
            if (lotId) createdLotIds.add(lotId);
          });
        }

        const diff = Number(item?.diff);
        const itemSku = skuKey(item?.sku);
        if (!(Number.isFinite(diff) && diff > 0) || !eventDate || !itemSku) return;

        const key = `${eventDate}\u0000${itemSku}`;
        if (!positiveChangesByDateAndSku.has(key)) positiveChangesByDateAndSku.set(key, []);
        positiveChangesByDateAndSku.get(key).push({
          qty: safeQtyInt(diff),
          referenceUnitPrice: safeFloat(item?.referenceUnitPrice)
        });
      });
    });
  });

  _adjustmentHistoryCache = {
    historyRef: history,
    createdLotIds,
    positiveChangesByDateAndSku
  };
  return _adjustmentHistoryCache;
}

function getAdjustmentCreatedLotIds() {
  return getAdjustmentHistoryCache().createdLotIds;
}
...
function isAdjustmentCreatedLotByHeuristic(lot) {
  ...
  const cacheKey = `${lotDate}\u0000${lotSku}`;
  const candidates = getAdjustmentHistoryCache().positiveChangesByDateAndSku.get(cacheKey) || [];
  return candidates.some(item => {
    if (!isSameMoneyValue(item?.referenceUnitPrice, lotPrice)) return false;
    return safeQtyInt(item?.qty) === lotInitialQty;
  });
}
...
function openPartDetailsModal(sku) {
  ...
  if (backdrop && panel) {
    openManagedModal({
      backdropId: "partDetailsBackdrop",
      panelId: "partDetailsPanel",
      bodyClass: "part-details-open",
      initialFocusId: "partDetailsCloseBtn",
      requestClose: closePartDetailsModal
    });
  }
}

function closePartDetailsModal() {
  closeManagedModal("partDetailsBackdrop");
  currentPartDetailsSku = null;
}

// === Batch preview by price and supplier ===
...
function openBatchPreviewByPrice(sku, price) {
  ...
  if (backdrop && panel) {
    openManagedModal({
      backdropId: "batchPreviewBackdrop",
      panelId: "batchPreviewPanel",
      bodyClass: "batch-preview-open",
      initialFocusId: "batchPreviewCloseBtn",
      requestClose: closeBatchPreviewModal
    });
  }
}

function closeBatchPreviewModal() {
  closeManagedModal("batchPreviewBackdrop");
}
    ...
    return `<tr>
      ...
      <td class="text-right">
        <div class="table-actions table-actions-tight">
          <button class="btn btn-danger btn-sm btn-icon" type="button" data-action="removeDeliveryItem" data-id="${escapeHtml(String(i.id))}" aria-label="Usuń">
...
function renderBuild() {
  ...
  els.buildItems.innerHTML = state.currentBuild.items.map(i => {
    ...
    return `<tr>
      ...
      <td class="text-right">
        <div class="table-actions table-actions-tight">
          <button class="btn btn-danger btn-sm btn-icon" type="button" data-action="removeBuildItem" data-id="${escapeHtml(String(i.id))}" aria-label="Usuń">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
          </button>
        </div>
      </td>
          ...
          return `
            <div class="lot-row">
              ...
              <input type="number" class="manual-lot-input"
                data-lot-id="${escapeHtml(String(lot?.id ?? ''))}"
                data-sku="${skuKeyStr}"
...
function getHistoryView() {
  const v = safeLocalStorageGet("magazyn_history_view_v3", null);
...
function parseHistoryDateRange(raw) {
  ...
  if (!value) return { fromISO: null, toISO: null, valid: true };

  const parts = value.split("-").map(part => part.trim());
  if (parts.length > 2) {
    return { fromISO: null, toISO: null, valid: false };
  }

  const rawFrom = parts[0] || "";
  const rawTo = parts.length === 2 ? (parts[1] || "") : "";
  const fromISO = rawFrom ? parsePLDateToISO(rawFrom) : null;
  const toISO = rawTo ? parsePLDateToISO(rawTo) : null;
  const hasAnyBoundary = !!rawFrom || !!rawTo;
  const valid = hasAnyBoundary
    && (!rawFrom || !!fromISO)
    && (!rawTo || !!toISO)
    && (!fromISO || !toISO || fromISO <= toISO);

  return {
    fromISO: valid ? fromISO : null,
    toISO: valid ? toISO : null,
    valid
  };
}
...
function historyMatchesFilters(ev, view, qNorm, fromISO, toISO, authorKey) {
  ...
  const d = ev.dateISO || "";
  if ((fromISO || toISO) && !d) return false;
...
function renderHistory() {
  ...
  const dateRange = parseHistoryDateRange(dateInput?.value || "");
  const { fromISO, toISO } = dateRange;
  const authorKey = normalize(authorSelect?.value || "");

  if (dateInput) {
    const invalid = dateRange.valid === false;
    dateInput.classList.toggle("input-invalid", invalid);
    dateInput.setAttribute("aria-invalid", invalid ? "true" : "false");
    dateInput.title = invalid
      ? "Podaj poprawną datę DD.MM.RRRR albo zakres DD.MM.RRRR-DD.MM.RRRR."
      : "";
  }
  ...
  tbody.innerHTML = paginatedHistory.rows.map(ev => {
    ...
    return `
      <tr data-hid="${escapeHtml(String(ev.id))}">
        ...
        <td class="text-right">
          <div class="table-row-actions">
            <button class="btn btn-secondary btn-sm" type="button" 
                data-action="toggleHistory" data-hid="${escapeHtml(String(ev.id))}">Szczegóły</button>
          </div>
        </td>
      </tr>
    `;
...
function openSupplierCatalogDetailsModal(supplierNameRaw) {
  ...
  openManagedModal({
    backdropId: 'supplierCatalogDetailsBackdrop',
    panelId: 'supplierCatalogDetailsPanel',
    bodyClass: 'catalog-readonly-open',
    initialFocusId: 'supplierCatalogDetailsCloseBtn',
    requestClose: closeSupplierCatalogDetailsModal
  });
}

function closeSupplierCatalogDetailsModal() {
  closeManagedModal('supplierCatalogDetailsBackdrop');
}
...
function openCatalogPartDetailsModal(skuRaw) {
  ...
  openManagedModal({
    backdropId: 'catalogPartDetailsBackdrop',
    panelId: 'catalogPartDetailsPanel',
    bodyClass: 'catalog-readonly-open',
    initialFocusId: 'catalogPartDetailsCloseBtn',
    requestClose: closeCatalogPartDetailsModal
  });
}

function closeCatalogPartDetailsModal() {
  closeManagedModal('catalogPartDetailsBackdrop');
}
...
function openMachineCatalogDetailsModal(machineCodeRaw) {
  ...
  openManagedModal({
    backdropId: 'machineCatalogDetailsBackdrop',
    panelId: 'machineCatalogDetailsPanel',
    bodyClass: 'catalog-readonly-open',
    initialFocusId: 'machineCatalogDetailsCloseBtn',
    requestClose: closeMachineCatalogDetailsModal
  });
}

function closeMachineCatalogDetailsModal() {
  closeManagedModal('machineCatalogDetailsBackdrop');
}
...
// === Supplier Prices UI ===
const _partSupplierPriceDrafts = {
  new: new Map(),
  edit: new Map()
};

function capturePartSupplierPriceDrafts(body, draftMap) {
  body?.querySelectorAll?.('input[data-sup]')?.forEach(input => {
    const supplierName = normalize(input.getAttribute('data-sup'));
    if (supplierName) draftMap.set(supplierName, String(input.value ?? ''));
  });
}

function resetPartSupplierPriceDrafts(mode = null) {
  if (!mode || mode === 'new') _partSupplierPriceDrafts.new.clear();
  if (!mode || mode === 'edit') _partSupplierPriceDrafts.edit.clear();
}

function bindSupplierPricesUI() {
  if (window.__supplierPricesUIBound) return;
  window.__supplierPricesUIBound = true;
...
function syncNewPartSupplierPricesUI() {
  ...
  capturePartSupplierPriceDrafts(body, _partSupplierPriceDrafts.new);
  ...
  body.innerHTML = selected.map(sup => `
    <div class="form-row" style="display:flex;align-items:center;gap:var(--space-3);margin-bottom:var(--space-2)">
      ...
      <input type="number" data-sup="${escapeHtml(sup)}" min="0" step="0.01" value="${escapeHtml(_partSupplierPriceDrafts.new.get(sup) ?? '0')}" 
...
function syncEditPartSupplierPricesUI() {
  ...
  capturePartSupplierPriceDrafts(body, _partSupplierPriceDrafts.edit);
  const selected = comboMultiGetSelected(checklist);
  ...
  body.innerHTML = selected.map(sup => {
    ...
    const savedPrice = supData?.prices?.get(sku) ?? 0;
    const currentPrice = _partSupplierPriceDrafts.edit.has(sup)
      ? _partSupplierPriceDrafts.edit.get(sup)
      : savedPrice;
    return `
      <div class="form-row" style="display:flex;align-items:center;gap:var(--space-3);margin-bottom:var(--space-2)">
        <span style="min-width:150px;font-size:var(--text-sm)">${escapeHtml(sup)}</span>
        <input type="number" data-sup="${escapeHtml(sup)}" min="0" step="0.01" value="${escapeHtml(currentPrice)}" 
          style="max-width:120px" placeholder="Cena">
        <span class="text-muted" style="font-size:var(--text-sm)">PLN</span>
      </div>
    `;
...
function openHistoryPreviewModal(ev) {
  ...
  if (backdrop && panel) {
    openManagedModal({
      backdropId: 'historyPreviewBackdrop',
      panelId: 'historyPreviewPanel',
      bodyClass: 'history-preview-open',
      initialFocusId: 'historyPreviewCloseBtn',
      requestClose: closeHistoryPreviewModal
    });
  }
}

function closeHistoryPreviewModal() {
  closeManagedModal('historyPreviewBackdrop');
}

// === Delegated table and catalog actions ===
document.addEventListener('click', (e) => {
  const paginationBtn = e.target?.closest?.('[data-action="changeTablePage"]');
  if (paginationBtn) {
    const tableKey = paginationBtn.getAttribute('data-table-key');
    const delta = Number(paginationBtn.getAttribute('data-delta'));
    if (tableKey && Number.isFinite(delta)) changeTablePage(tableKey, delta);
    return;
  }

  const removeDeliveryBtn = e.target?.closest?.('[data-action="removeDeliveryItem"]');
  if (removeDeliveryBtn) {
    const id = Number(removeDeliveryBtn.getAttribute('data-id'));
    if (Number.isFinite(id) && typeof window.removeDeliveryItem === 'function') window.removeDeliveryItem(id);
    return;
  }

  const removeBuildBtn = e.target?.closest?.('[data-action="removeBuildItem"]');
  if (removeBuildBtn) {
    const id = Number(removeBuildBtn.getAttribute('data-id'));
    if (Number.isFinite(id) && typeof window.removeBuildItem === 'function') window.removeBuildItem(id);
    return;
  }

  const removeBomBtn = e.target?.closest?.('[data-action="removeBomItem"]');
  if (removeBomBtn) {
    const index = Number(removeBomBtn.getAttribute('data-index'));
    if (Number.isInteger(index) && index >= 0 && typeof window.removeBomItem === 'function') window.removeBomItem(index);
    return;
  }

  const supplierEditBtn = e.target?.closest?.('[data-action="openSupplierEditor"]');
  if (supplierEditBtn) {
    const supplier = supplierEditBtn.getAttribute('data-supplier');
    if (supplier && typeof window.openSupplierEditor === 'function') window.openSupplierEditor(supplier);
    return;
  }

  const supplierArchiveBtn = e.target?.closest?.('[data-action="toggleSupplierArchive"]');
  if (supplierArchiveBtn) {
    const supplier = supplierArchiveBtn.getAttribute('data-supplier');
    if (supplier && typeof window.toggleSupplierArchive === 'function') void window.toggleSupplierArchive(supplier);
    return;
  }

  const partEditBtn = e.target?.closest?.('[data-action="startEditPart"]');
  if (partEditBtn) {
    const sku = partEditBtn.getAttribute('data-sku');
    if (sku && typeof window.startEditPart === 'function') window.startEditPart(sku);
    return;
  }

  const partArchiveBtn = e.target?.closest?.('[data-action="togglePartArchive"]');
  if (partArchiveBtn) {
    const sku = partArchiveBtn.getAttribute('data-sku');
    if (sku && typeof window.togglePartArchive === 'function') void window.togglePartArchive(sku);
    return;
  }

  const machineEditBtn = e.target?.closest?.('[data-action="openMachineEditor"]');
  if (machineEditBtn) {
    const code = machineEditBtn.getAttribute('data-machine-code');
    if (code && typeof window.openMachineEditor === 'function') window.openMachineEditor(code);
    return;
  }

  const machineArchiveBtn = e.target?.closest?.('[data-action="toggleMachineArchive"]');
  if (machineArchiveBtn) {
    const code = machineArchiveBtn.getAttribute('data-machine-code');
    if (code && typeof window.toggleMachineArchive === 'function') void window.toggleMachineArchive(code);
    return;
  }
...
// Close only the topmost modal on Escape and keep keyboard focus inside it.
document.addEventListener('keydown', (e) => {
  const topModal = _managedModalStack[_managedModalStack.length - 1] || null;
  if (e.key === 'Tab' && topModal) {
    const focusable = getManagedModalFocusableElements(topModal);
    if (!focusable.length) {
      e.preventDefault();
      focusManagedModal(topModal);
      return;
    }
  ...
  if (requestCloseTopManagedModal() || closeTopmostPreviewModal()) {
    e.preventDefault();
    e.stopImmediatePropagation();
  }
});
